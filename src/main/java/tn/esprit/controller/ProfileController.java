...
public class ProfileController implements Initializable {
    ...
    public  Personne user = Session.getUser();
    ...
    public void onDelete() {
        ...
        try {
            // 1. Modification de la méthode supprimer pour retourner un boolean
            boolean isDeleted = serviceUser.supprimer(user);

            if(isDeleted) {
                user = null;
                gui.getInstance().getViewFactory().closeStage(stage);
                showAlert("Suppression réussie", "L'utilisateur a été supprimé", Alert.AlertType.INFORMATION);
                gui.getInstance().getViewFactory().showLoginWindow();
            } else {
                showAlert("Échec de la suppression", "Aucun utilisateur trouvé", Alert.AlertType.ERROR);
            }

        } catch (Exception e) {
            showAlert("Erreur critique", e.getMessage(), Alert.AlertType.ERROR);
